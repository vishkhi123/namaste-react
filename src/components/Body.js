...
const Body = () => {
  ...
  const fetchData = async () => {
    ...
    setFilteredList( json?.data.success?.cards[1]?.gridWidget?.gridElements?.infoWithStyle
        ?.restaurants)
  };
    ...
    <div className="body">
      <div className="filter">
        <div className="search">
          <input type="text" className="search-bar" onChange={(e)=>setSearchBar(e.target.value)} value={searchBar}></input>

        

          <button  className="search-btn" onClick={()=>{
               const filteredList= listOfRestautants.filter((e)=>(
                        e.info.name.includes(searchBar)
